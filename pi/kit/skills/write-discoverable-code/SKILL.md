...
description: Rules for writing code that agents and humans can find by plain-text search. Apply when writing or renaming code.
license: MIT
---
...
hits. They have no hover text, no jump-to-definition, and no memory between sessions. Every
identifier is a search query and every search miss costs wasted reads. These rules make code
resolvable in one search instead of five.
...
## Frontcode amendment (Joel, 2026-09-03)

Adopted whole with one exception: tests stay in each package's `tests/` folder, not colocated. The boundary lint and `src/README.md` depend on that layout.
